import type { Messages } from './en.js';
import type { Tools } from './tools.js';

export const es = (p: Tools): Messages => ({
    comum: {
        aCarregar: 'Cargando…',
        aGuardar: 'Guardando…',
        guardar: 'Guardar',
        naoFoiPossivel: 'No ha funcionado. Inténtalo de nuevo.',
        idioma: 'Idioma',
    },

    nav: {
        avisos: 'Avisos',
        crews: 'Crews',
        servidores: 'Servidores',
        asMinhas: 'Las mías',
        perfil: 'Perfil',
        principal: 'Navegación principal',
        recrutamento: 'Reclutamiento',
        forum: 'Foro',
        premium: 'Precios',
        entrar: 'Entrar',
        sair: 'Salir',
    },

    landing: {
        aAcontecer: 'Ocurriendo en ViceHub',
        aDecorrerAgora: 'Ocurriendo ahora',
        comecaEm: (quando: string) => `Empieza ${quando}`,
        naCrew: (nome: string) => `con ${nome}`,
        titulo: 'Tu crew. Tu servidor. Tu parte.',
        subtitulo:
            'ViceHub es donde las comunidades de juego se organizan: dirige tu crew, mueve lo que ganáis y demuestra quién apareció de verdad.',
        criarConta: 'Crear mi cuenta',
        verRecrutamento: 'Quién está reclutando',
        jaTenhoConta: 'Ya tengo cuenta',

        quemRecruta: 'Crews buscando gente',
        agoraOnline: 'Servidores en línea ahora',
        verTudo: 'Ver todos',
        jogadores: (n: number) =>
            p.plural(n, { one: '1 jugador', other: `${n} jugadores` }),
        planosTitulo: 'Cuánto cuesta',
        planosGratis:
            'Jugar es gratis, y lo seguirá siendo: tu perfil, tu banner, postularte a crews, ser miembro, programar eventos, presentarte. Lo que se paga es mover el dinero.',
        planoCrew: 'Crew',
        planoCrewPreco: '4,99 \u20ac / mes',
        planoCrewTexto:
            'Una tesorería que cuadra: proponer, aprobar y repartir lo que gana la crew — todos cobran a la vez, o ninguno. Treinta días para probarlo.',
        planoServidor: 'Servidor',
        planoServidorPreco: 'desde 14,99 \u20ac / mes',
        planoServidorTexto:
            'Hasta 10 crews jugando en tu servidor, y una tesorería para pagarles. Más crews, plan mayor.',
        crewsTitulo: 'Crews y servidores',
        crewsTexto:
            'Reúne a tu gente con rangos que significan algo. Las solicitudes se responden, no se ignoran.',

        tesourariaTitulo: 'Una tesorería que cuadra',
        tesourariaTexto:
            'Movimientos por propuesta y aprobación, con cuatro saldos para que nadie comprometa dos veces el mismo dinero. O cobran todos, o no cobra nadie.',

        eventosTitulo: 'Se cobra por aparecer',
        eventosTexto:
            'Hacéis el trabajo, se confirma quién estuvo, y se reparte por asistencia — con pesos, porque quien lidera un golpe suele llevarse más.',

        verPremium: 'Qué te da el premium',
        honesto: 'Todavía al principio',
        honestoTexto:
            'ViceHub se está construyendo a la vista, empezando por GTA VI. Hay cosas listas y otras no, y la forma más rápida de cambiarlo es que nos digas qué falta.',
    },
    auth: {
        ouEntao: 'o',
        entrarComDiscord: 'Continuar con Discord',
        entrarComGoogle: 'Continuar con Google',
        entrarTitulo: 'Entrar',
        entrarSub: 'Bienvenido de vuelta a ViceHub.',
        email: 'Email',
        password: 'Contraseña',
        aEntrar: 'Entrando…',
        credenciaisErradas: 'Ese correo y esa contraseña no coinciden.',
        contaBloqueada: 'Demasiados intentos fallidos. Inténtalo dentro de un rato.',
        esqueciPassword: 'He olvidado mi contraseña',
        criarConta: 'Crear cuenta',
        jaTensConta: '¿Ya tienes cuenta?',

        registoTitulo: 'Crear cuenta',
        registoSub: 'Lleva menos de un minuto.',
        nomeJogador: 'Nombre de jugador',
        aCriar: 'Creando…',
        passwordMinima: (n: number) => `Al menos ${n} caracteres.`,
        captchaNaoCarregou:
            'La comprobación de que no eres un robot no cargó. Recarga la página para intentarlo de nuevo: sin ella no se puede continuar.',
        emailOcupado: 'Ya existe una cuenta con este correo.',
        nomeOcupado: 'Ese nombre ya está ocupado. Elige otro.',
        naoFoiPossivelCriar: 'No se ha podido crear la cuenta.',

        recuperarTitulo: 'Recuperar la contraseña',
        recuperarSub:
            'Dinos la dirección de la cuenta y te enviamos un enlace para definir una contraseña nueva.',
        enviarLink: 'Enviar el enlace',
        aEnviar: 'Enviando…',
        verificaEmail: 'Revisa tu correo',
        seExistir:
            'Si existe una cuenta con esa dirección, el enlace ya va en camino. Sirve una vez y caduca en una hora.',
        voltarAoLogin: 'Volver al inicio de sesión',
        jaMeLembro: 'Ya me acuerdo',

        novaPasswordTitulo: 'Contraseña nueva',
        novaPasswordSub:
            'Al guardar, se cierran todas las sesiones abiertas de esta cuenta — incluida la de quien no debería estar.',
        novaPassword: 'Contraseña nueva',
        guardarPassword: 'Guardar la contraseña',
        linkNaoServe: 'Este enlace ya no sirve. Pide otro.',
        pedirOutroLink: 'Pedir otro enlace',
        naoFoiPossivelPassword: 'No se ha podido definir la contraseña.',

        confirmarEmailTitulo: 'Confirmar el correo',
        aConfirmar: 'Confirmando…',
        emailConfirmado: 'Dirección confirmada. Tu cuenta está lista.',
        linkSemCodigo:
            'Esta dirección no trae ningún código. Abre el enlace tal como llegó en el correo.',
        irParaViceHub: 'Ir a ViceHub',
    },

    crews: {
        titulo: 'Crews',
        criar: 'Crear crew',
        procurar: 'Buscar por nombre o etiqueta',
        procurarLabel: 'Buscar crews',
        botaoProcurar: 'Buscar',
        emDestaque: 'Destacadas',
        destaque: 'Destacada',
        aRecrutarAgora: 'Buscando gente',
        todas: 'Todas las crews',
        resultados: (termo: string) => `Resultados para "${termo}"`,
        semResultados: 'Ninguna crew con ese nombre. Prueba otro término.',
        aindaNaoHa: 'Todavía no hay crews. Crea la primera.',
        anterior: 'Anterior',
        seguinte: 'Siguiente',
        paginaDe: (atual: number, total: number) => `Página ${atual} de ${total}`,
        ordenarPor: 'Ordenar por',
        ordemRecentes: 'Más recientes',
        ordemNivel: 'Nivel',
        ordemNome: 'Nombre',
        lugar: (posicao: number, total: number) =>
            `#${posicao} entre ${total} crews clasificadas`,
        naoCarregou: 'No se ha podido cargar el directorio.',
        nivel: (n: number) => `Nivel ${n}`,
        membros: (n: number) =>
            p.plural(n, { one: '1 miembro', other: `${n} miembros` }),

        criarTitulo: 'Crear crew',
        criarSub: 'Serás el líder, y podrás invitar a quien quieras después.',
        nome: 'Nombre',
        nomeAjuda: 'Entre 3 y 48 caracteres.',
        tag: 'Etiqueta',
        tagAjuda:
            'Entre 2 y 8 letras o números. Aparece junto al nombre, así: [VICE].',
        descricao: 'Descripción',
        caracteresDisponiveis: (n: number) =>
            p.plural(n, {
                one: 'Opcional. Queda 1 carácter.',
                other: `Opcional. Quedan ${n} caracteres.`,
            }),
        criarBotao: 'Crear la crew',
        nomeOcupado: 'Ya existe una crew con este nombre.',
        tagOcupada: 'Esa etiqueta ya está ocupada. Elige otra.',
        naoFoiPossivelCriar: 'No se ha podido crear la crew.',
        voltarDiretorio: 'Volver al directorio',

        naoEncontrada: 'No hemos encontrado esta crew.',
        xp: 'XP',
        contagemMembros: 'Miembros',
        recruta: 'Reclutando',
        recrutaTitulo: 'Esta crew está reclutando',
        recrutaDesde: (data: string) => `Reclutando desde ${data}.`,
        recrutaLabel: 'Decir que esta crew está reclutando',
        recrutaAjuda:
            'Pone la crew en el tablón de reclutamiento y la marca en el directorio. No cambia nada más: siguen teniendo que pedirlo, y sigues decidiendo tú. Desactívalo cuando estés completo — el tablón muestra cuánto lleva puesto cada anuncio.',
        recrutamentoTitulo: 'Crews reclutando',
        recrutamentoExplica: 'Solo las crews que dijeron que buscan gente.',
        verQuemRecruta: 'Ver qué crews están reclutando',
        verTodas: 'Ver todas las crews',
        requisitos: 'Lo que esta crew te pide',
        requisitosAjuda:
            'Una condición por línea. Nadie se verifica contra esto: sirve para que quien se postula sepa a qué atenerse, y quien decide quién entra sigues siendo tú.',
        cartaLabel: 'Diles quién eres',
        cartaPlaceholder: 'Edad, a qué horas juegas, qué buscas…',
        cartaAjuda:
            'Opcional, pero es lo único que leerán antes de decidir. Quien manda en la crew ve esto y nadie más.',
        pedirEntrada: 'Pedir entrar',
        responderamQueNao: 'Respondieron que no',
        candidaturaRecusada: 'Esta vez no',
        procuram: 'Buscan',
        respostaNova: 'Nueva',
        entrasteAgora: 'Ya estás dentro',
        enviadaEm: (data: string) => `Enviada el ${data}`,
        candidaturaEnviada: 'Solicitud enviada',
        retirarCandidatura: 'Retirar la solicitud',
        sair: 'Salir de la crew',
        tesouraria: 'Tesorería',
        eventos: 'Eventos',
        entraParaCandidatar: 'para solicitar entrar en esta crew.',
        entraLink: 'Entra',
        candidaturasPorResponder: 'Solicitudes sin responder',
        aceitar: 'Aceptar',
        recusar: 'Rechazar',
        remover: 'Expulsar',
        listaMembros: 'Miembros',
        cargoDe: (nome: string) => `Rol de ${nome}`,
        oQueOCargoDa:
            'El rol decide qué puede hacer cada uno aquí. Los oficiales proponen y deciden sobre el dinero; los líderes cambian además los roles. El tuyo no lo cambias tú, y al último líder no se le puede quitar el cargo.',

        asMinhasTitulo: 'Dónde pertenezco',
        minhasCrews: 'Crews',
        meusServidores: 'Servidores',
        semComunidades: 'Todavía no perteneces a ninguna crew ni servidor.',
        naoCarregouMinhas: 'No se han podido cargar tus crews.',
        procuraUma: 'Busca una',
        ouCriaTua: 'o crea la tuya',
        definicoes: 'Ajustes de la crew',
        definicoesGuardadas: 'Ajustes de la crew guardados.',
        nomeJaExiste: 'Ya existe una crew con este nombre.',
        planoVemDoServidor: 'El plan que cubre a esta crew viene del servidor',
        planoAtivo:
            'El plan de la crew está activo. El banner y el color aparecen en su página pública.',
        precisaDePlano:
            'Estos campos son del plan premium, y el plan es de la crew, no tuyo. Puedes rellenarlos, pero solo se guardan mientras la crew tenga uno.',
        verPremium: 'Ver qué le da el premium a la crew',
        aEsperaResposta: 'Esperando respuesta',
    },

    cargos: {
        crew_leader: 'Líder',
        crew_officer: 'Oficial',
        crew_member: 'Miembro',
        server_owner: 'Dueño',
        server_moderator: 'Moderador',
        server_member: 'Miembro',
    },

    servidores: {
        titulo: 'Servidores',
        registar: 'Registrar servidor',
        procurar: 'Buscar por nombre',
        procurarLabel: 'Buscar servidores',
        soOnline: 'Mostrar solo los que están en línea',
        online: 'En línea',
        offline: 'Desconectado',

        actividadeTitulo: 'Quién estuvo aquí, hora a hora',

        mediaDeSete: 'de media, últimos 7 días',

        picoDeSete: 'en la hora más llena',

        semDados: 'nada reportado',

        mediaDe: (n: number) => `${n} de media`,

        ordens: {
            newest: 'Más recientes',
            active: 'Donde está la gente',
        },

        tiraDescricao: (horas: number, media: number, pico: number) =>
            `Cuántas personas estuvieron aquí en las últimas ${horas} horas. ${media} de media en los últimos 7 días, ${pico} en la hora más llena.`,
        todos: 'Todos los servidores',
        semResultados: 'Ningún servidor con estos filtros.',
        aindaNaoHa: 'Todavía no hay servidores. Registra el primero.',
        naoCarregou: 'No se han podido cargar los servidores.',
        registarTitulo: 'Registrar servidor',
        registarSub: 'Serás el dueño, y podrás aceptar a quien solicite entrar.',
        regiao: 'Región',
        regiaoAjuda:
            'Opcional. Ayuda a quien busca baja latencia, por ejemplo Europa.',
        registarBotao: 'Registrar el servidor',
        nomeOcupado: 'Ya existe un servidor con este nombre.',
        naoFoiPossivelRegistar: 'No se ha podido registrar el servidor.',
        naoEncontrado: 'No hemos encontrado este servidor.',
        sair: 'Salir del servidor',
        definicoes: 'Ajustes del servidor',
        definicoesGuardadas: 'Ajustes del servidor guardados.',
        nomeJaExiste: 'Ya existe un servidor con este nombre.',
        nome: 'Nombre',
        descricao: 'Descripción',
        requisitos: 'Lo que este servidor te pide',
        requisitosAjuda:
            'Una condición por línea. Nadie se comprueba contra esto: sirve para decir qué esperar antes de postularse, y quien decide quién entra sigues siendo tú.',
        reportaPorSi:
            'Este servidor informa por sí mismo: estar en línea viene de su última señal, y no de una casilla marcada aquí.',
        estaOnline: 'El servidor está online ahora mismo',
        planoAtivo:
            'El plan del servidor está activo. El banner y el color aparecen en su página pública.',
        precisaDePlano:
            'Estos campos son del plan premium, y el plan es del servidor, no tuyo. Puedes rellenarlos, pero solo se guardan mientras el servidor tenga uno.',
        verPremium: 'Ver qué le da el premium al servidor',
        entraParaCandidatar: 'para solicitar entrar en este servidor.',
    },

    conquistas: {
        titulo: 'Logros',
        nomes: {
            attended_1: 'Se presentó',
            attended_10: 'Diez noches',
            attended_50: 'Cincuenta noches',
            ran_1: 'Organizó un evento',
            ran_10: 'Organizó diez eventos',
            ran_50: 'Organizó cincuenta eventos',
            paid_1: 'Pagó a los suyos',
            paid_10: 'Pagó diez veces',
            paid_50: 'Pagó cincuenta veces',
            level_5: 'Nivel 5',
            level_10: 'Nivel 10',
            level_25: 'Nivel 25',
        },
    },

    pendentes: {
        titulo: 'Lo que te espera',
        pedidosDeEntrada: (n: number) =>
            p.plural(n, {
                one: '1 solicitud de entrada',
                other: `${n} solicitudes de entrada`,
            }),
        pedidosDeFiliacao: (n: number) =>
            p.plural(n, {
                one: '1 crew pidiendo jugar allí',
                other: `${n} crews pidiendo jugar allí`,
            }),
        decisoesDeDinheiro: (n: number) =>
            p.plural(n, {
                one: '1 decisión esperando en la tesorería',
                other: `${n} decisiones esperando en la tesorería`,
            }),
        amizades: 'Amistades',
        pedidosDeAmizade: (n: number) =>
            p.plural(n, {
                one: '1 solicitud de amistad',
                other: `${n} solicitudes de amistad`,
            }),
        porResponder: (n: number) =>
            p.plural(n, { one: '1 esperando', other: `${n} esperando` }),
    },

    planos: {
        premium: 'Crew',
        server_base: 'Servidor',
        server_plus: 'Servidor +',
        server_unlimited: 'Servidor sin límite',
    },

    legal: {
        atualizado: (iso: string) => `Actualizado el ${p.data(iso)}`,
        rascunho: 'Borrador — aún no está en vigor',
        rascunhoTexto:
            'A este documento le faltan los datos de la empresa que hay detrás de ViceHub, así que todavía no obliga a nadie. Se publica para poder leerse y revisarse, no para basarse en él.',
        idioma:
            'Publicado en inglés. Cualquier traducción se ofrece por cortesía; el texto que rige es el inglés.',
        termos: 'Términos',
        privacidade: 'Privacidad',
        rodape: 'Legal',
        marcas:
            'ViceHub es una plataforma independiente, sin vínculo ni respaldo de Rockstar Games, Take-Two Interactive ni ninguna editora. Los nombres y marcas de los juegos pertenecen a sus dueños.',
    },

    erros: {

        CAPTCHA_FAILED: 'La comprobación de que no eres un robot no pasó. Inténtalo de nuevo.',

        INVALID_CREDENTIALS: 'Ese correo y esa contraseña no coinciden.',

        ACCOUNT_LOCKED: 'Demasiados intentos fallidos. Inténtalo dentro de un rato.',

        EMAIL_ALREADY_EXISTS: 'Ya existe una cuenta con este correo.',

        USERNAME_ALREADY_EXISTS: 'Ese nombre está ocupado. Elige otro.',

        INVALID_ACCESS_TOKEN: 'Tu sesión ya no es válida. Entra de nuevo.',

        INVALID_REFRESH_TOKEN: 'Tu sesión caducó. Entra de nuevo.',

        SESSION_EXPIRED: 'Tu sesión caducó. Entra de nuevo.',

        REFRESH_TOKEN_REUSED: 'Esta sesión se cerró por seguridad. Entra de nuevo.',

        SESSION_NOT_FOUND: 'Esta sesión ya no existe.',

        USER_NOT_FOUND: 'No encontramos a este jugador.',

        INVALID_ACCOUNT_TOKEN: 'Este enlace ya no funciona. Pide otro.',

        EMAIL_ALREADY_VERIFIED: 'Esta dirección ya está confirmada.',

        ACCOUNT_DELETION_NOT_CONFIRMED: 'La confirmación no coincide. Escribe tu nombre tal como aparece.',

        ACCOUNT_LEADS_COMMUNITIES: 'Todavía lideras una crew o un servidor. Pasa el liderazgo o ciérralo primero.',

        ACCOUNT_HAS_ACTIVE_PLAN: 'Todavía tienes un plan activo. Cancélalo antes de eliminar la cuenta.',

        FEDERATED_NOT_CONFIGURED: 'Esta forma de entrar todavía no está disponible.',

        FEDERATED_UNAVAILABLE: 'No pudimos contactar con ese servicio. Inténtalo dentro de un rato.',

        FEDERATED_EXCHANGE_FAILED: 'Entrar por ahí no funcionó. Inténtalo de nuevo.',

        FEDERATED_STATE_MISMATCH: 'Esta entrada tardó demasiado o empezó en otro sitio. Empieza de nuevo.',

        FEDERATED_EMAIL_UNUSABLE: 'Esa cuenta no tiene una dirección de correo utilizable.',

        INSUFFICIENT_PERMISSIONS: 'No tienes permiso para hacer eso.',

        LAST_ROLE_HOLDER: 'Esta es la última persona con ese cargo. Dáselo a otra persona primero.',

        CREW_NOT_FOUND: 'No encontramos esta crew.',

        CREW_NAME_TAKEN: 'Ya existe una crew con este nombre.',

        CREW_TAG_TAKEN: 'Esa etiqueta está ocupada. Elige otra.',

        SERVER_NOT_FOUND: 'No encontramos este servidor.',

        SERVER_NAME_TAKEN: 'Ya existe un servidor con este nombre.',

        MEMBERSHIP_NOT_FOUND: 'No encontramos esa solicitud.',

        ALREADY_MEMBER: 'Ya eres miembro.',

        NOT_A_MEMBER: 'Solo los miembros pueden hacer eso.',

        MEMBERSHIP_NOT_PENDING: 'Esta solicitud ya fue respondida.',

        CANNOT_MANAGE_SELF: 'No puedes hacerte eso a ti mismo.',

        CREW_HAS_FUNDS: 'La tesorería de la crew todavía tiene dinero. Vacíala primero.',

        SERVER_HAS_FUNDS: 'La tesorería del servidor todavía tiene dinero. Vacíala primero.',

        CREW_HAS_OPEN_DECISIONS: 'Todavía hay decisiones pendientes. Resuélvelas primero.',

        SERVER_HAS_OPEN_DECISIONS: 'Todavía hay decisiones pendientes. Resuélvelas primero.',

        CREW_HAS_ACTIVE_PLAN: 'La crew todavía tiene un plan activo. Cancélalo primero.',

        SERVER_HAS_ACTIVE_PLAN: 'El servidor todavía tiene un plan activo. Cancélalo primero.',

        AFFILIATION_NOT_FOUND: 'No encontramos esa afiliación.',

        CREW_ALREADY_AFFILIATED: 'Esta crew ya juega en un servidor.',

        AFFILIATION_ALREADY_REQUESTED: 'Esta crew ya pidió jugar aquí.',

        AFFILIATION_NOT_PENDING: 'Esta petición ya fue respondida.',

        SERVER_CREW_LIMIT_REACHED: 'Este servidor tiene tantas crews como permite su plan.',

        EVENT_NOT_FOUND: 'No encontramos este evento.',

        INVALID_EVENT_OWNER: 'Este evento es de otra comunidad.',

        EVENT_NOT_SCHEDULED: 'Solo puedes apuntarte a un evento que no ha empezado.',

        EVENT_ALREADY_CLOSED: 'Este evento ya está cerrado.',

        INVALID_STATUS_TRANSITION: 'Este evento ya no está en un estado que permita eso.',

        EVENT_FULL: 'Este evento está lleno.',

        ALREADY_SIGNED_UP: 'Ya estás apuntado.',

        NOT_SIGNED_UP: 'Este jugador no se apuntó a este evento.',

        ATTENDANCE_NOT_CONFIRMABLE: 'No se confirman asistencias en un evento cancelado.',

        STARTS_IN_THE_PAST: 'Un evento no puede empezar en el pasado.',

        ENDS_BEFORE_IT_STARTS: 'Un evento no puede acabar antes de empezar.',

        WALLET_NOT_FOUND: 'No encontramos esa cartera.',

        INVALID_WALLET_OWNER: 'Esa cartera es de otra persona.',

        MOVEMENT_NOT_FOUND: 'No encontramos ese movimiento.',

        MOVEMENT_NOT_PENDING: 'Este movimiento ya fue decidido.',

        INSUFFICIENT_FUNDS: 'No hay disponible suficiente para esto.',

        BALANCE_WOULD_OVERFLOW: 'Ese importe es mayor de lo que una tesorería puede guardar.',

        NOT_THE_PROPOSER: 'Solo quien lo propuso puede retirarlo.',

        DISTRIBUTION_NOT_FOUND: 'No encontramos ese reparto.',

        DISTRIBUTION_NOT_PENDING: 'Este reparto ya fue decidido.',

        NO_MEMBERS_TO_PAY: 'No hay nadie a quien pagar.',

        EVENT_NOT_IN_THIS_TREASURY: 'Ese evento no es de esta comunidad.',

        NO_CONFIRMED_PARTICIPANTS: 'Nadie tuvo la asistencia confirmada en ese evento.',

        SHARES_DO_NOT_MATCH_TOTAL: 'Las partes no suman el total.',

        CREW_DOES_NOT_PLAY_HERE: 'Esa crew no juega en este servidor.',

        SUBSCRIPTION_REQUIRED: 'Esto necesita un plan activo.',

        INVALID_SUBSCRIPTION_OWNER: 'Ese plan es de otra persona.',

        SUBSCRIPTION_OWNER_NOT_FOUND: 'No encontramos de quién es ese plan.',

        SUBSCRIPTION_NOT_FOUND: 'No encontramos ese plan.',

        SUBSCRIPTION_ALREADY_CANCELED: 'Este plan ya está marcado para terminar.',

        SUBSCRIPTION_ALREADY_ENDED: 'Este plan ya terminó.',

        LIFETIME_HAS_NO_DURATION: 'Un plan vitalicio no tiene fecha de fin.',

        LIFETIME_CANNOT_BE_CANCELED: 'Un plan vitalicio no se cancela. Se revoca.',

        ALREADY_LIFETIME: 'Esta cuenta ya tiene un plan vitalicio.',

        BILLING_NOT_CONFIGURED: 'Los pagos todavía no están configurados. Inténtalo más tarde.',

        BILLING_OWNER_NOT_FOUND: 'No encontramos de quién es este pago.',

        STRIPE_REQUEST_FAILED: 'El servicio de pagos no respondió. Inténtalo dentro de un rato.',

        SUBSCRIPTION_NOT_FROM_STRIPE: 'Este plan no se compró aquí, así que no se gestiona aquí.',

        PLAN_IS_FOR_COMMUNITIES: 'Ese plan es para una crew o un servidor, no para una persona.',

        PLAN_NOT_PURCHASABLE: 'Ese plan no está a la venta.',

        PLAN_WRONG_OWNER: 'Ese plan no sirve para este tipo de titular.',

        API_KEY_NOT_FOUND: 'No encontramos esa clave.',

        INVALID_API_KEY: 'Esa clave fue rechazada.',

        TOO_MANY_API_KEYS: 'Este servidor ya tiene tantas claves como puede. Revoca una primero.',

        CANNOT_FRIEND_SELF: 'No puedes añadirte a ti mismo.',

        ALREADY_FRIENDS: 'Ya sois amigos.',

        ALREADY_REQUESTED: 'Ya lo has pedido.',

        FRIENDSHIP_NOT_FOUND: 'No encontramos esa petición.',

        FRIENDSHIP_NOT_PENDING: 'Esta petición ya fue respondida.',

        CANNOT_ACCEPT_OWN_REQUEST: 'No puedes aceptar tu propia petición.',

        TOPIC_NOT_FOUND: 'Esta pregunta no existe o fue retirada.',

        REPLY_NOT_FOUND: 'Esta respuesta no existe o fue retirada.',

        TOPIC_LOCKED: 'Esta pregunta está cerrada a nuevas respuestas.',

        NOT_YOURS: 'Solo quien lo escribió puede tocarlo.',

        LISTING_NOT_FOUND: 'Este anuncio no existe o fue retirado.',

        NOT_ON_SERVER: 'Solo quien juega en este servidor puede vender aquí.',

        ALREADY_CLOSED: 'Este anuncio ya no está a la venta.',

        CONVERSATION_NOT_FOUND: 'No encontramos esta conversación.',

        MESSAGE_NOT_FOUND: 'Este mensaje no existe o fue retirado.',

        REVIEW_NOT_FOUND: 'Esta valoración no existe o fue retirada.',

        NOT_SOLD: 'Solo se valora una venta que ocurrió.',

        NO_DEAL: 'Solo valora quien habló con quien vendió sobre este anuncio.',

        ALREADY_REVIEWED: 'Ya valoraste esta venta.',

        ALREADY_REPLIED: 'Ya respondiste a esta valoración.',

        SELLER_GONE: 'Quien lo anunció ya no tiene cuenta.',

        IS_YOURS: 'Esto es tuyo. Usa el botón de retirar.',

    },

    avisos: {
        titulo: 'Lo que te perdiste',

        sub: 'Cosas que otras personas hicieron y que te afectan. Nada de esto va por correo — eso todavía no está hecho.',

        vazio: 'No tienes nada esperando.',

        naoCarregou: 'No se pudieron cargar tus avisos.',

        darTodosPorLidos: 'Marcar todo como leído',

        naoFoiPossivelLer: 'No se pudo marcar como leído.',

        porLer: (quantos: string) => `${quantos} esperando`,

        oQueAconteceu: {
            market_message: (quem: string) => `${quem} te escribió sobre`,
            forum_reply: (quem: string) => `${quem} respondió a`,
            market_review: (quem: string) => `${quem} valoró tu venta de`,
            market_review_reply: (quem: string) =>
                `${quem} respondió a tu valoración de`,
        },

        alguem: 'Alguien',
    },

    mercado: {
        titulo: 'Mercado',

        sub: (servidor: string) => `Lo que se vende en ${servidor}.`,

        moedaDeJogo:
            'Los precios de aquí son en moneda del juego. ViceHub no mueve dinero real, y vender cualquier cosa por dinero de verdad va contra los Términos.',

        anunciar: 'Vender algo',

        entrarParaAnunciar: 'Entra para vender algo',

        aindaSemAnuncios: 'Todavía no hay nada a la venta. Pon lo primero.',

        nadaNesteFiltro: 'Aquí no hay nada así.',

        naoCarregou: 'No se pudo cargar el mercado.',

        naoEncontrado: 'No encontramos este anuncio.',

        voltar: 'Volver al mercado',

        verMercado: 'Mercado',

        categorias: {
            vehicle: 'Vehículos',
            property: 'Propiedades',
            business: 'Negocios',
            service: 'Servicios',
            item: 'Objetos',
            other: 'Otra cosa',
        },

        categoriaLabel: 'Categoría',

        todasAsCategorias: 'Todo',

        estados: {
            open: 'A la venta',
            sold: 'Vendido',
            withdrawn: 'Retirado',
        },

        tituloDoAnuncio: 'Qué vendes, en una línea',

        corpoDoAnuncio:
            'Descríbelo: en qué estado está, dónde lo entregas, qué aceptas por ello',

        preco: 'Precio',

        precoAjuda: 'Moneda del juego. Números enteros, sin puntos ni espacios.',

        precoInvalido: 'El precio son cifras, y nada más.',

        imagem: 'Dirección de una imagen',

        imagemAjuda:
            'Opcional. ViceHub no aloja imágenes — pega la dirección de una.',

        publicar: 'Ponerlo a la venta',

        naoFoiPossivelPublicar: 'No se pudo publicar el anuncio.',

        editar: 'Editar',

        guardar: 'Guardar los cambios',

        cancelar: 'Cancelar',

        naoFoiPossivelGuardar: 'No se pudieron guardar los cambios.',

        marcarVendido: 'Marcar como vendido',

        marcarRetirado: 'Quitarlo del mercado',

        naoFoiPossivelFechar: 'No se pudo cambiar eso.',

        retirar: 'Borrar el anuncio',

        confirmarRetirar: '¿Borrar este anuncio para siempre?',

        naoFoiPossivelRetirar: 'No se pudo borrar el anuncio.',

        vendidoEm: (quando: string) => `Vendido el ${quando}`,

        retiradoEm: (quando: string) => `Retirado el ${quando}`,

        porQuem: 'Vendido por',

        contaApagada: 'una cuenta borrada',

        quantos: (n: number) =>
            n === 1 ? '1 cosa a la venta' : `${n} cosas a la venta`,

        conversas: 'Mensajes',

        conversasPrivadas:
            'Cada una de estas es entre tú y otra persona. Nadie más las lee — un moderador solo ve un mensaje si uno de vosotros lo denuncia.',
        conversaPrivada:
            'Esta conversación es entre vosotros dos. Nadie más la lee — un moderador solo ve un mensaje si uno de vosotros lo denuncia.',

        perguntar: 'Preguntar por esto',

        entrarParaPerguntar: 'Entra para preguntar por esto',

        semConversas: 'Todavía no hay mensajes.',

        naoCarregouConversas: 'No se pudieron cargar tus mensajes.',

        conversaNaoEncontrada: 'No encontramos esta conversación.',

        escreverMensagem: 'Escribir un mensaje',

        enviarMensagem: 'Enviar',

        naoFoiPossivelEnviar: 'No se pudo enviar el mensaje.',

        anuncioRetirado: 'Este anuncio fue retirado. La conversación se sigue leyendo, pero está cerrada a mensajes nuevos.',

        retiradaComAConta: 'Retirado con la cuenta.',

        retirarMensagem: 'Retirar',

        naoFoiPossivelRetirarMensagem: 'No se pudo retirar el mensaje.',

        voltarAsConversas: 'Volver a los mensajes',

        sobreOAnuncio: 'Sobre',

        avaliacoes: 'Valoraciones',

        avaliacaoPublica:
            'Las valoraciones son públicas. Solo valora quien habló con quien vendió sobre un anuncio que se vendió, y quien vendió tiene derecho a una respuesta.',

        semAvaliacoes: 'Todavía no hay valoraciones.',

        naoCarregouAvaliacoes: 'No se pudieron cargar las valoraciones.',

        avaliar: 'Valorar esta venta',

        aNota: 'Tu nota',

        oComentario: 'Qué pasó (opcional)',

        enviarAvaliacao: 'Dejar la valoración',

        naoFoiPossivelAvaliar: 'No se pudo dejar la valoración.',

        responderAvaliacao: 'Responder',

        aResposta: 'Tu respuesta, una sola',

        enviarResposta: 'Enviar la respuesta',

        naoFoiPossivelResponder: 'No se pudo enviar la respuesta.',

        retirarAvaliacao: 'Retirar',

        naoFoiPossivelRetirarAvaliacao: 'No se pudo retirar la valoración.',

        media: (nota: number, quantas: number) =>
            quantas === 1
                ? `${nota} de 5, de 1 valoración`
                : `${nota} de 5, de ${quantas} valoraciones`,

        estrelas: (nota: number) => `${nota} de 5`,

        sobreAVenda: 'Sobre',
    },

    moderacao: {
        denunciar: 'Denunciar',
        porqueDenuncias: '¿Qué le pasa?',
        razoes: {
            spam: 'Spam o publicidad',
            abuse: 'Insultos, acoso u odio',
            off_topic: 'No es para esto que sirve este sitio',
            other: 'Otra cosa',
        },
        notaDaDenuncia: 'Lo que un moderador debería saber (opcional)',
        enviarDenuncia: 'Enviar la denuncia',
        cancelarDenuncia: 'Cancelar',
        denunciaRecebida: 'Denunciado. Un moderador lo verá.',
        naoFoiPossivelDenunciar: 'No se pudo enviar la denuncia.',
        naoFoiPossivelDecidir: 'No se pudo guardar eso.',
        filaTitulo: 'Denuncias',
        filaSub: 'Las más antiguas primero, porque son las que más han esperado.',
        filaEstados: {
            open: 'En espera',
            acted: 'Atendidas',
            dismissed: 'Sin motivo',
        },
        filaVazia: 'Aquí no hay nada.',
        filaNegada: 'Esto es para moderadores.',
        irParaFila: 'Denuncias en espera',
        marcarTratada: 'Me ocupé de esto',
        marcarSemRazao: 'Descartar',
        jaTratada: 'Atendida.',
        jaDispensada: 'Vista, sin nada malo.',
        /** As três espécies de coisa que caem na fila. */
        alvos: {
            topic: 'Una pregunta',
            reply: 'Una respuesta',
            listing: 'Un anuncio',
            message: 'Un mensaje en una conversación',
            review: 'Una valoración',
        },

        verOAlvo: 'Abrir',
    },

    forum: {

        titulo: 'Foro',

        subtitulo: 'Pregunta, responde y ayudaos entre vosotros.',

        perguntar: 'Hacer una pregunta',

        aindaSemPerguntas: 'Todavía no hay preguntas. Haz la primera.',

        naoCarregou: 'No se pudo cargar el foro.',

        naoEncontrada: 'No encontramos esta pregunta.',

        tituloDaPergunta: 'Tu pregunta, en una línea',

        corpoDaPergunta: 'Di qué intentaste y qué pasó',

        publicar: 'Publicar la pregunta',

        naoFoiPossivelPublicar: 'No se pudo publicar la pregunta.',

        responder: 'Escribir una respuesta',

        enviarResposta: 'Publicar la respuesta',

        naoFoiPossivelResponder: 'No se pudo publicar la respuesta.',

        semRespostas: 'Todavía sin respuestas.',

        retirar: 'Retirar',

        naoFoiPossivelRetirar: 'No se pudo retirar eso.',

        retiradoComAConta: 'Este texto se retiró junto con la cuenta de quien lo escribió.',

        contaApagada: 'una cuenta eliminada',

        fechada: 'Cerrada a nuevas respuestas',

        fechar: 'Cerrar a respuestas',

        reabrir: 'Reabrir a respuestas',

        naoFoiPossivelFechar: 'No se pudo cambiar eso.',

        voltar: 'Volver al foro',

        entrarParaPerguntar: 'Entra para hacer una pregunta',

        jaRetirado: 'Ya fue retirado.',

    },

    noticias: {

        titulo: 'Lo que pasa en el juego',

    },

    perfil: {
        apagarConta: 'Eliminar mi cuenta',
        apagarContaExplicacao:
            'Puedes irte cuando quieras. La cuenta y todo lo que te identifica desaparecen; lo que tus crews necesitan para que sus cuentas cuadren se queda, con un nombre que no dice nada de ti.',
        apagarContaFica:
            'El correo, el nombre, la imagen y el texto se borran, y también la contraseña y los accesos por Discord o Google. Lo que se queda: los movimientos que propusiste o aprobaste, y los eventos a los que asististe — eso es de tus crews y no tuyo, y quitarlo dejaría sus cuentas sin cuadrar.',
        apagarContaPerdeSaldo: (saldo: string) =>
            `Tu cartera tiene ${saldo}. Borrar la cuenta lo pierde: no hay forma de enviarlo a ninguna parte antes, y no vuelve.`,
        apagarContaPassword: 'Tu contraseña, si la cuenta tiene una',
        apagarContaConfirmar: 'Eliminar la cuenta definitivamente',
        terminarSessoes: 'Cerrar sesión en todas partes',
        terminarSessoesExplicacao:
            'Cierra todas las sesiones en todos los dispositivos, este incluido: también saldrás de aquí. Sirve si sospechas que alguien entró en tu cuenta: cambiar la contraseña no echa a quien ya está dentro.',
        terminarSessoesConfirmar: 'Cerrar todas las sesiones',
        terminarSessoesATerminar: 'Cerrando…',
        terminarSessoesFalhou:
            'No se pudieron cerrar las sesiones. Inténtalo dentro de un rato.',
        primeiroPassoTitulo: 'Empieza por aquí',
        primeiroPassoExplicacao:
            'ViceHub gira en torno a una crew: su gente, sus eventos y el dinero que gana. Todavía no estás en ninguna: crea una y lleva a los tuyos, o busca una que esté reclutando.',
        primeiroPassoCriar: 'Crear una crew',
        primeiroPassoProcurar: 'Ver quién está reclutando',
        levarDados: 'Llévate tus datos',
        levarDadosExplicacao:
            'Un archivo con todo lo que ViceHub tiene sobre ti: la cuenta, las comunidades en las que estás, los eventos a los que asististe, lo que propusiste y decidiste en una tesorería, y lo que la plataforma te dio. La contraseña y tus accesos no van ahí — eso son llaves de la cuenta, no datos sobre ti.',
        levarDadosBotao: 'Descargar mis datos',
        levarDadosAPreparar: 'Preparando…',
        levarDadosFalhou: 'No se pudo preparar el archivo. Inténtalo en un momento.',
        titulo: 'Mi perfil',
        verPublico: 'Ver como público',
        naoCarregou: 'No se ha podido cargar tu perfil.',
        jogador: 'Jugador',
        nivel: 'Nivel',
        reputacao: 'Reputación',
        reputacaoDeOnde: 'De dónde viene tu reputación',
        reputacaoVazia: 'Todavía nada. La reputación viene de los eventos: presentarte a uno en el que alguien te confirmó da un punto, apuntarte y no presentarte quita uno.',
        reputacaoApareceu: 'te presentaste',
        reputacaoFaltou: 'no te presentaste',
        reputacaoEventoApagado: 'un evento que ya no existe',
        conta: 'Cuenta',
        confirmado: 'Confirmado',
        porConfirmar: 'Sin confirmar',
        plano: 'Plan',
        semPlano: 'Sin plan',
        premiumVitalicio: 'Premium de por vida',
        premiumAte: (data: string) => `Premium hasta ${data}`,
        enviarConfirmacao: 'Enviar correo de confirmación',
        emailEnviado: 'Correo enviado',
        naoFoiPossivelEmail: 'No se ha podido enviar el correo de confirmación.',
        apresentacao: 'Presentación',
        sobreTi: 'Sobre ti',
        avatar: 'Avatar',
        perfilGuardado: 'Perfil guardado.',
        naoFoiPossivelPerfil: 'No se ha podido guardar el perfil.',
        personalizacao: 'Personalización',
        premium: 'Premium',
        banner: 'Banner',
        cor: 'Color de acento',
        corAjuda: 'Hexadecimal de seis dígitos, como #E93CEF.',
        guardarPersonalizacao: 'Guardar personalización',
        personalizacaoGuardada: 'Personalización guardada.',
        ehPremium: 'La personalización forma parte del plan premium.',
        naoFoiPossivelPersonalizacao:
            'No se ha podido guardar la personalización.',
        naoEncontrado: 'No hemos encontrado a este jugador.',
        desde: 'Desde',
        irParaCrews: 'Ir a las crews',
    },

    premium: {
        etiqueta: 'Premium',
        titulo: 'El plan es de la comunidad, no tuyo',
        subtitulo:
            'Jugar en ViceHub es gratis, y lo sigue siendo. Lo que se paga es gestionar una comunidad: mover el dinero que gana la crew, y tener crews jugando en tu servidor.',
        tituloComunidade: (nome: string) => `Premium para ${nome}`,
        subtituloCrew:
            'El plan es de la crew, no de quien lo paga. Mientras esté activo, quien gestiona la crew puede mover su dinero — y definir su banner y su color.',
        crewCobertaPeloServidor: (servidor: string) =>
            `Esta crew ya está cubierta por el plan de ${servidor}, el servidor donde juega. Lo mantiene mientras juegue allí.`,
        crewTemPlano: 'Esta crew ya tiene un plan activo.',
        irParaCrew: 'Ir a la crew',
        subtituloServidor:
            'El plan es del servidor, no de quien lo paga. Decide cuántas crews pueden jugar allí, y abre la tesorería del servidor.',
        servidorDaPersonalizacao:
            'Paga a las crews que juegan en tu servidor, directamente desde su tesorería.',
        servidorDaEquipa:
            'Un plan para todo el servidor, y para las crews que juegan allí. No depende de quién pagó.',
        servidorTemPlano: 'Este servidor ya tiene un plan activo.',
        irParaServidor: 'Ir al servidor',
        aTesourariaDaCrew: 'La tesorería de una crew',
        escolheEscalao: 'Elige tu nivel',
        ateCrews: (n: number) =>
            p.plural(n, {
                one: 'Hasta 1 crew jugando en tu servidor',
                other: `Hasta ${n} crews jugando en tu servidor`,
            }),
        semLimiteDeCrews: 'Sin límite de crews jugando allí',
        todosPorMes: 'Todos los precios son por mes.',
        porMes: 'al mes',
        planoEDeComunidade:
            'Aquí no hay nada que comprar para tu cuenta. El plan es de una crew o de un servidor: abre la que gestionas y cómpralo allí.',
        asMinhasComunidades: 'Mis comunidades',
        oQueDaPersonalizacao:
            'Una tesorería que cuadra: una crew propone, aprueba y reparte lo que gana — todos cobran a la vez, o ninguno.',
        oQueDaCrew:
            'Postularse, pertenecer, programar eventos y presentarse siguen siendo gratis para todos. Tu perfil también.',
        crewDaPersonalizacao:
            'Una tesorería que cuadra: proponer, aprobar y repartir lo que gana la crew — todos cobran a la vez, o ninguno.',
        crewDaEquipa:
            'Un plan para toda la crew. Quien la gestiona puede mover el dinero: no depende de quién pagó.',
        oQueDaApoio:
            'Es lo que mantiene la plataforma en pie, y paga el tiempo que esto lleva.',
        comprar: 'Hazte premium',
        aAbrir: 'Abriendo el pago…',
        cancelarQuando: 'Cancela cuando quieras. Sigue hasta el final del mes que pagaste.',
        criarConta: 'Crear cuenta',
        jaTenhoConta: 'Ya tengo cuenta',
        tensPlano: 'Tu plan está activo.',
        tensPlanoAte: (data: string) => `Tu plan está activo hasta el ${data}.`,
        tensVitalicio: 'Tienes premium vitalicio. No termina y nunca se cobra.',
        irParaPerfil: 'Ir a mi perfil',
        aindaNaoAbriu:
            'La compra todavía no está abierta: abre pronto. Mientras tanto, todo lo demás en ViceHub funciona con normalidad.',
        naoFoiPossivelComprar: 'No se pudo abrir el pago. Inténtalo dentro de un rato.',
        gerirPlano: 'Gestionar plan',
        gerirOndeSeCancela:
            'Cancela, cambia la tarjeta o descarga las facturas. Cancelar detiene la siguiente renovación: el periodo que ya pagaste sigue hasta el final.',
        naoFoiPossivelGerir:
            'No se pudo abrir la página de facturación. Inténtalo dentro de un rato.',
        notaVitalicio:
            'El vitalicio no está a la venta. Se da a mano, uno a uno, a quien apoyó ViceHub al principio.',
    },

    carteira: {
        titulo: 'Mi cartera',
        dondeVem:
            'Lo que una crew te pagó aterriza aquí. Es para verlo, no para moverlo: no hay forma de enviarlo a ninguna parte, y nada aquí finge lo contrario.',
        aindaSemNada: 'Todavía no ha llegado nada a tu cartera.',
        verCarteira: 'Mi cartera',
    },

    rasto: {
        titulo: 'Lo que se decidió aquí',
        aindaNada: 'Todavía no se ha decidido nada sobre nadie.',
        contaApagada: 'una cuenta borrada',
        alguem: 'alguien',
        semCargo: 'sin rol',
        admitiu: (quem: string, alvo: string) => `${quem} dejó entrar a ${alvo}`,
        recusou: (quem: string, alvo: string) => `${quem} rechazó a ${alvo}`,
        removeu: (quem: string, alvo: string, cargo: string) =>
            `${quem} expulsó a ${alvo}, que era ${cargo}`,
        mudouCargo: (quem: string, alvo: string, de: string, para: string) =>
            `${quem} pasó a ${alvo} de ${de} a ${para}`,
        aceitouCrew: (quem: string, crew: string) =>
            `${quem} dejó que ${crew} jugara aquí`,
        foiAceiteEm: (quem: string, servidor: string) =>
            `${quem} dejó que esta crew jugara en ${servidor}`,
        recusouCrew: (quem: string, crew: string) =>
            `${quem} rechazó a ${crew}`,
        foiRecusadaEm: (quem: string, servidor: string) =>
            `${quem} rechazó a esta crew en ${servidor}`,
        tirouCrew: (quem: string, crew: string) =>
            `${quem} echó a ${crew} de este servidor`,
        foiTiradaDe: (quem: string, servidor: string) =>
            `${quem} echó a esta crew de ${servidor}`,
        fez: (quem: string, acao: string) => `${quem}: ${acao}`,
    },

    tesouraria: {
        montanteAEnviar: 'Importe a enviar',
        transferirTitulo: 'Enviar dinero a una crew',
        transferirAviso:
            'Lo que gana un servidor financia a las crews que juegan allí. Esto se mueve ya — es tu propia tesorería pagando.',
        paraQueCrew: 'A qué crew',
        escolheCrew: 'Elige una crew',
        semCrewsNoServidor:
            'Todavía no juega aquí ninguna crew. Acepta una y podrás financiarla desde aquí.',
        transferir: 'Enviar',
        aTransferir: 'Enviando…',
        transferida: 'Enviado.',
        totalADividir: 'Importe a repartir',
        dividirTitulo: 'Repartir lo que ganó la crew',
        dividirAviso:
            'Nada se mueve hasta que alguien con poder para decidir lo apruebe — como en cualquier otro movimiento.',
        comoDividir: 'Cómo repartirlo',
        deQueEvento: 'De qué evento',
        escolheEvento: 'Elige un evento',
        semEventosComPresencas:
            'Todavía no hay asistencias confirmadas en ningún evento. Confirma quién apareció en un evento y podrás repartir por ahí.',
        presencas: (n: number) =>
            p.plural(n, { one: '1 apareció', other: `${n} aparecieron` }),
        nota: 'Nota',
        dividir: 'Proponer el reparto',
        aDividir: 'Proponiendo…',
        divisaoProposta: 'Reparto propuesto. Queda a la espera de una decisión.',
        titulo: 'Tesorería',
        verCrew: 'Ver la crew',
        precisaDePlano:
            'Mover dinero forma parte del plan de la crew — el plan es de la crew, no tuyo. El saldo, el extracto y los repartos pasados quedan a la vista de todos; proponer y aprobar sí requieren un plan activo.',
        verPlano: 'Ver lo que el plan da a la crew',
        avaliacaoAcaba: (dias: number) =>
            p.plural(dias, {
                one: 'Tu prueba acaba mañana. Después el saldo sigue a la vista, pero proponer y aprobar requieren un plan.',
                other: `Tu prueba acaba en ${dias} días. Después el saldo sigue a la vista, pero proponer y aprobar requieren un plan.`,
            }),
        soParaMembros:
            'Las cuentas de una crew solo son visibles para sus miembros.',
        disponivel: 'Disponible',
        liquidado: 'Liquidado',
        aEntrar: 'Por entrar',
        aSair: 'Por salir',
        explicacaoDisponivel:
            'El disponible ya descuenta las salidas sin decidir. Ese es el número que dice cuánto se puede comprometer sin contar dos veces el mismo dinero.',
        proporTitulo: 'Proponer un movimiento',
        proporAviso:
            'Proponer no mueve nada. Queda pendiente hasta que alguien con autoridad lo apruebe.',
        montante: 'Importe',
        montanteAjuda: 'En unidades enteras de la moneda del juego.',
        direcao: 'Dirección',
        entrada: 'Entrada',
        saida: 'Salida',
        categoria: 'Categoría',
        descricao: 'Descripción',
        propor: 'Proponer el movimiento',
        aPropor: 'Proponiendo…',
        proposto: 'Movimiento propuesto. Queda a la espera de decisión.',
        extrato: 'Extracto',
        semMovimentos: 'Todavía no hay movimientos.',
        aprovar: 'Aprobar',
        recusar: 'Rechazar',
        cancelar: 'Cancelar',
        aprovado: 'Movimiento aprobado.',
        recusado: 'Movimiento rechazado.',
        cancelado: 'Propuesta cancelada.',
        pagar: 'Pagar',
        divisaoPaga: 'Reparto pagado. La parte de cada uno está en su cartera.',
        naoFoiPossivel: 'No se ha podido completar la operación.',
        divisoes: 'Repartos de ganancias',
        pessoas: (n: number) =>
            p.plural(n, { one: '1 persona', other: `${n} personas` }),
    },

    categorias: {
        contribution: 'Contribución',
        server_costs: 'Costes del servidor',
        marketing: 'Marketing',
        event: 'Evento',
        prize: 'Premio',
        service: 'Servicio',
        payout: 'Pago',
        other: 'Otro',
    },

    estadosMovimento: {
        pending: 'Sin decidir',
        approved: 'Aprobado',
        rejected: 'Rechazado',
        canceled: 'Cancelado',
    },

    bases: {
        equal: 'A partes iguales',
        by_role: 'Ponderado por rango',
        manual: 'Importes indicados uno a uno',
        participation: 'Por quien apareció',
    },

    eventos: {
        quemVe: 'Quién lo ve',
        naMontra: 'En la página de inicio',
        soAComunidade: 'Solo esta comunidad',
        porNaMontra: 'Mostrar en la página de inicio',
        tirarDaMontra: 'Quitar de la página de inicio',
        montraAjuda:
            'Desactivado por defecto. Activado, el nombre, la hora y la comunidad aparecen a quien abra ViceHub — nunca quién se apuntó.',
        publicado: 'Este evento ya aparece en la página de inicio.',
        tirado: 'Este evento ya no aparece en la página de inicio.',
        titulo: 'Eventos',
        verComunidade: 'Volver a la comunidad',
        soParaMembros:
            'Este calendario solo es visible para los miembros de la comunidad.',
        mostrarPassados: 'Mostrar también los que ya pasaron',
        todos: 'Todos los eventos',
        oQueVem: 'Lo que viene',
        semEventos: 'Nada programado. Programa el primero aquí abajo.',
        semHistorico: 'Todavía no ha habido eventos aquí.',
        marcarTitulo: 'Programar un evento',
        nome: 'Nombre',
        comeca: 'Empieza',
        lugares: 'Plazas',
        lugaresAjuda: 'Déjalo vacío para que no haya límite.',
        descricao: 'Descripción',
        marcar: 'Programar el evento',
        aMarcar: 'Programando…',
        marcado: 'Evento programado.',
        soQuemGere: 'Programar eventos es cosa de quien dirige la comunidad.',
        naoFoiPossivelMarcar: 'No se ha podido programar el evento.',
        inscritos: (n: number) =>
            p.plural(n, { one: '1 inscrito', other: `${n} inscritos` }),
        deLugares: (n: number) => ` de ${n}`,
        comPresenca: (n: number) =>
            p.plural(n, {
                one: '1 con asistencia confirmada',
                other: `${n} con asistencia confirmada`,
            }),

        naoEncontrado: 'No hemos encontrado este evento.',
        todosOsEventos: 'Todos los eventos',
        estado: 'Estado',
        contagemInscritos: 'Inscritos',
        confirmados: 'Confirmados',
        inscreverMe: 'Apuntarme',
        inscrito: 'Inscrito',
        retirarInscricao: 'Retirar la inscripción',
        presencaConfirmada: (peso: number) =>
            `Asistencia confirmada · peso ${peso}`,
        inscricaoFeita:
            'Inscripción hecha. La asistencia la confirma quien organiza.',
        inscricaoRetirada: 'Inscripción retirada.',
        soQuemOrganiza: 'Eso es cosa de quien organiza el evento.',
        quemSeInscreveu: 'Quién se ha apuntado',
        diferenca:
            'Apuntarse y tener la asistencia confirmada son cosas distintas. Solo quien organiza puede afirmar que alguien estuvo allí, y es esa afirmación — no la inscripción — la que da derecho a cobrar en el reparto por participación de la comunidad.',
        semInscricoes: 'Todavía no hay inscripciones.',
        pesoDe: (nome: string) => `Peso de ${nome}`,
        confirmarPresenca: 'Confirmar asistencia',
        naoApareceu: 'No apareció',
        presencaDe: (nome: string) => `Asistencia de ${nome} confirmada.`,
        ausente: (nome: string) => `${nome} marcado como ausente.`,
        comecar: 'Empezar',
        terminar: 'Terminar',
        cancelarEvento: 'Cancelar',
        estadoMudou: (estado: string) => `Evento: ${estado.toLowerCase()}.`,
        jaPodeDividir: (n: number) =>
            p.plural(n, {
                one: '1 asistencia confirmada. La crew ya puede repartir ganancias por participación a partir de este evento.',
                other: `${n} asistencias confirmadas. La crew ya puede repartir ganancias por participación a partir de este evento.`,
            }),
    },

    filiacao: {
        titulo: 'Servidor',
        jogaEm: 'Esta crew juega en',
        semServidor: 'Esta crew todavía no juega en ningún servidor.',
        procurar: 'Buscar un servidor',
        semResultados: 'Ningún servidor coincide con eso.',
        pedir: 'Pedir jugar aquí',
        pedidoEnviado: 'Esperando respuesta de',
        desistir: 'Retirar la solicitud',
        sair: 'Salir del servidor',
        crewsDoServidor: 'Crews',
        semCrews: 'Todavía no hay crews jugando en este servidor.',
        pedidos: 'Crews que han pedido jugar aquí',
        aceitar: 'Aceptar',
        recusar: 'Rechazar',
        remover: 'Quitar',
        crewsDoPlano: (usadas: number, limite: number) =>
            `${usadas} de las ${limite} crews que permite tu plan`,
        crewsSemLimite: (usadas: number) =>
            p.plural(usadas, {
                one: '1 crew juega aquí. Tu plan no pone límite.',
                other: `${usadas} crews juegan aquí. Tu plan no pone límite.`,
            }),
        planoCheio:
            'Ya tienes todas las crews que permite tu plan. Las que juegan aquí se quedan; aceptar otra necesita un plan mayor.',
        verEscaloes: 'Ver los planes para servidores',
    },

    chaves: {
        titulo: 'Claves del servidor',
        comoInstalar: 'Cómo se instala',
        paraQueServem:
            'Una clave permite que tu servidor informe a ViceHub — si está en pie y con cuánta gente dentro. Instala el recurso en el servidor y pega ahí la clave.',
        nome: 'Nombre de la clave',
        nomeExemplo: 'producción',
        criar: 'Crear una clave',
        copiaAgora:
            'Cópiala ahora. Es la única vez que aparece — de este lado queda un resumen, no la clave.',
        revogar: 'Revocar',
        revogada: 'revocada',
        nuncaUsada: 'nunca usada',
        usadaEm: (quando: string) => `usada el ${quando}`,
        aindaNenhuma: 'Este servidor todavía no tiene claves.',
        jogadoresOnline: (quantos: number) =>
            quantos === 1 ? '1 jugador en línea' : `${quantos} jugadores en línea`,
    },

    feed: {
        titulo: 'Lo que ha pasado',
        aindaNada:
            'Todavía nada. Entra en una crew, organiza un evento o añade un amigo — esto se llena solo.',
        eventoFeito: (xp: number) => `+${xp} XP de un evento ·`,
        entrouEm: 'entró en',
        naoCarregou: 'No se ha podido cargar esto.',
    },

    amigos: {
        titulo: 'Amigos',
        adicionar: 'Añadir',
        aceitar: 'Aceptar',
        recusar: 'Rechazar',
        retirar: 'Retirar',
        desfazer: 'Eliminar',
        saoAmigos: 'Amigos',
        pedidoEnviado: 'Solicitud enviada',
        pedidos: 'Esperándote',
        aindaNenhum: 'Aún no tienes amigos. Abre el perfil de alguien y pídelo.',
        desde: (quando: string) => `Amigos desde ${quando}`,
        naoCarregou: 'No se han podido cargar tus amigos.',
    },

    progressao: {
        nivelAtual: (nivel: number) => `Nivel ${nivel}`,
        faltam: (xp: string, proximo: number) =>
            `Faltan ${xp} XP para el nivel ${proximo}`,
        noTopo: (nivel: number) =>
            `Nivel ${nivel} — el máximo que hay, por ahora.`,
        historico: 'De dónde viene este nivel',
        aindaSemGanhos:
            'Todavía no hay XP. Organiza un evento, confirma quién apareció y ciérralo.',
        eventoApagado: 'un evento que ya no existe',
        deOndeVem:
            'Una crew gana XP cuando un evento que organizó termina con gente que apareció de verdad. Quien apareció también gana.',
    },

    zonaPerigo: {

        aindaMandasEm: (nomes: string) => `Eres la única persona al mando de: ${nomes}. Pasa el cargo a otra persona, o elimina esas comunidades, antes de eliminar tu cuenta.`,
        crewTitulo: 'Eliminar esta crew',
        servidorTitulo: 'Eliminar este servidor',
        crewExplicacao:
            'La crew sale del directorio, sus miembros pierden la pertenencia y deja de estar afiliada al servidor donde juega. El nombre y la tag vuelven a quedar libres.',
        servidorExplicacao:
            'El servidor sale del directorio, sus miembros pierden la pertenencia, las crews que juegan allí dejan de estar afiliadas —incluidas las que cubría su plan— y sus claves dejan de servir. El nombre vuelve a quedar libre.',
        escreveONome: (nome: string) => `Escribe ${nome} para confirmar.`,
        confirmacao: 'Confirmación',
        botao: 'Eliminar',
        aApagar: 'Eliminando…',
        temSaldo: 'La tesorería aún tiene saldo. Repártelo o retíralo primero.',
        temDecisoes:
            'Hay movimientos o repartos pendientes de decisión en la tesorería.',
        temPlano: 'Hay un plan activo. Cancélalo primero.',
        naoFoiPossivel: 'No se ha podido eliminar.',
    },

    estadosEvento: {
        scheduled: 'Programado',
        ongoing: 'En curso',
        completed: 'Terminado',
        canceled: 'Cancelado',
    },

    participacao: {
        signed_up: 'Inscrito',
        confirmed: 'Asistencia confirmada',
        no_show: 'No apareció',
        withdrawn: 'Se retiró',
    },
});
