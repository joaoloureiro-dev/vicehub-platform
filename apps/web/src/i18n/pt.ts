...
export const pt = (p: Tools): Messages => ({
    ...
    erros: {
        ...
        CONVERSATION_NOT_FOUND: 'Não encontrámos esta conversa.',
    ...
    avisos: {
        titulo: 'O que te escapou',

        sub: 'Coisas que outras pessoas fizeram e que te dizem respeito. Nada disto vai por email — isso ainda não está feito.',

        vazio: 'Não tens nada à espera.',

        naoCarregou: 'Não foi possível carregar os teus avisos.',

        darTodosPorLidos: 'Dar tudo por lido',

        naoFoiPossivelLer: 'Não foi possível dar isso por lido.',

        porLer: (quantos: string) => `${quantos} à espera`,

        oQueAconteceu: {
            market_message: (quem: string) => `${quem} escreveu-te sobre`,
            forum_reply: (quem: string) => `${quem} respondeu a`,
    ...
    mercado: {
        ...
        conversasPrivadas:
            'Cada uma destas é entre ti e uma outra pessoa. Mais ninguém as lê — um moderador só vê uma mensagem se um de vocês a denunciar.',
        conversaPrivada:
            'Esta conversa é entre vocês os dois. Mais ninguém a lê — um moderador só vê uma mensagem se um de vocês a denunciar.',

        perguntar: 'Perguntar sobre isto',

        entrarParaPerguntar: 'Entra para perguntares sobre isto',

        semConversas: 'Ainda não há mensagens.',

        naoCarregouConversas: 'Não foi possível carregar as tuas mensagens.',

        conversaNaoEncontrada: 'Não encontrámos esta conversa.',

        escreverMensagem: 'Escrever uma mensagem',

        enviarMensagem: 'Enviar',

        naoFoiPossivelEnviar: 'Não foi possível enviar a mensagem.',

        anuncioRetirado: 'Este anúncio foi retirado. A conversa continua a ler-se, mas está fechada a mensagens novas.',

        retiradaComAConta: 'Retirada com a conta.',

        retirarMensagem: 'Retirar',

        naoFoiPossivelRetirarMensagem: 'Não foi possível retirar a mensagem.',
    ...
    moderacao: {
        ...
        razoes: {
            ...
            off_topic: 'Não é para isto que este sítio serve',
            other: 'Outra coisa',
        },
        ...
        verOAlvo: 'Abrir',
        verHistorial: 'O que já foi decidido antes',
        historialFalhou: 'Não foi possível carregar esse historial.',
        escreveu: (quem: string, retiradas: number, mantidas: number) =>
            `O que ${quem} escreveu: ${retiradas} retiradas, ${mantidas} vistas e deixadas ficar`,
        denunciou: (quem: string, comRazao: number, semRazao: number) =>
            `Denúncias que ${quem} apresentou: ${comRazao} com razão, ${semRazao} sem razão`,
        historialNota:
            'Contam-se coisas já decididas, e não denúncias recebidas: dez pessoas a denunciar a mesma publicação são uma decisão, e não dez.',
    },
